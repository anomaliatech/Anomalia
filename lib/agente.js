...
function completarLead(lead, servicio) {
  const l = Object.assign({}, lead || {});
  if (servicio && !l.servicio) l.servicio = servicio;
  return l;
}

// Devuelve las etiquetas de los campos obligatorios que siguen faltando.
function faltanObligatorios(negocio, lead) {
  return (negocio.camposLead || [])
    .filter((c) => c.obligatorio && !(lead && lead[c.id]))
    .map((c) => c.etiqueta);
}

module.exports = { instrucciones, herramientas, completarLead, faltanObligatorios };
