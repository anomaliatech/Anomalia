...
// Ordena huecos por cercanía a un instante objetivo (la fecha/hora que pidió el
// visitante): así, si su hora exacta está ocupada, lo más cercano de la lista son
// las horas más próximas de ESE día, y si el día entero está lleno, los huecos de
// los días más próximos — sin tener que programar esos dos casos por separado.
function ordenarPorCercania(fechas, objetivoMs) {
  if (objetivoMs == null || Number.isNaN(objetivoMs)) return fechas;
  return [...fechas].sort((a, b) => Math.abs(a.getTime() - objetivoMs) - Math.abs(b.getTime() - objetivoMs));
}

// Devuelve una lista de inicios de cita libres (objetos Date), respetando horario y
// ocupación. Si se da "preferencia" (Date o ISO string), se devuelven los más
// cercanos a ese instante primero; si no, los más próximos en el tiempo primero.
async function huecosLibres(negocio, { duracionMin, desde, preferencia, limite } = {}) {
  const zona = negocio.zonaHoraria;
  const tope = limite || 12;
  const objetivo = preferencia != null ? new Date(preferencia).getTime() : null;
  if (process.env.MODO_PRUEBA === 'si') {
    const base = objetivo != null && !Number.isNaN(objetivo) ? new Date(objetivo) : (() => {
      const f = new Date(Date.now() + 2 * 86400000); f.setUTCHours(8, 0, 0, 0); return f;
    })();
    return [-1, 0, 1].map((i) => {
      const f = new Date(base.getTime() + i * 3600000);
      return { inicio: f.toISOString(), iso: isoConZona(f, zona), etiqueta: etiquetaHumana(f, zona, negocio.idioma || 'es'), exacto: i === 0 && objetivo != null };
    });
  }
  ...
  // Se recogen TODOS los huecos libres de la ventana (no solo los primeros 12):
  // hace falta conocerlos todos para poder ordenar por cercanía a la preferencia
  // y no perderse un hueco más cercano que caiga más adelante en el calendario.
  const libres = [];
  for (let d = 0; d < (negocio.diasVistaPrevia || 10); d++) {
  ...
  // Hora exacta que pidió el visitante aunque no caiga en la rejilla de horas
  // en punto (p. ej. 14:30): vale si entra en el horario de ese día, respeta la
  // antelación y dura entera (dur) sin pisar nada ocupado del calendario.
  if (objetivo != null && !Number.isNaN(objetivo) && objetivo % 900000 === 0) {
    const iniPref = objetivo;
    const finPref = iniPref + dur * 60000;
    const pp = new Intl.DateTimeFormat('en-CA', { timeZone: zona, year: 'numeric', month: '2-digit', day: '2-digit', weekday: 'short' })
      .formatToParts(new Date(iniPref)).reduce((a, x) => ((a[x.type] = x.value), a), {});
    const enHorario = (negocio.horarioAtencion[DIA_EN[pp.weekday]] || []).some(([ini, fin]) => {
      const [hi, mi] = ini.split(':').map(Number);
      const [hf, mf] = fin.split(':').map(Number);
      const apertura = horaLocalAInstante(+pp.year, +pp.month, +pp.day, hi, mi, zona).getTime();
      const cierre = horaLocalAInstante(+pp.year, +pp.month, +pp.day, hf, mf, zona).getTime();
      return iniPref >= apertura && finPref <= cierre;
    });
    const chocaOcupado = ocupado.some(([bi, bf]) => iniPref < bf && finPref > bi);
    const yaEsta = libres.some((f) => f.getTime() === iniPref);
    if (enHorario && !chocaOcupado && !yaEsta
        && iniPref >= inicioVentana.getTime() && iniPref <= finVentana.getTime()) {
      libres.push(new Date(iniPref));
    }
  }

  return ordenarPorCercania(libres, objetivo).slice(0, tope)
    .map((f) => ({
      inicio: f.toISOString(),
      iso: isoConZona(f, zona),
      etiqueta: etiquetaHumana(f, zona, negocio.idioma || 'es'),
      exacto: objetivo != null && Math.abs(f.getTime() - objetivo) < 60000,
    }));
}
...
async function crearCita(negocio, { inicioISO, duracionMin, servicio, lead }) {
  ...
  const evento = await api(`/calendars/${encodeURIComponent(calId(negocio))}/events`, {
    method: 'POST',
    body: JSON.stringify({
      summary: `Cita: ${lead.nombre || 'Lead web'}${servicio ? ' - ' + servicio : ''}`,
      ...
      extendedProperties: {
        private: {
          [TAG]: 'si',
          recordatorio24Enviado: 'no',
          recordatorio2Enviado: 'no',
...
// campos: p.ej. { recordatorio24Enviado: 'si' } o los dos flags a la vez.
async function marcarRecordado(negocio, eventoId, campos) {
  await api(`/calendars/${encodeURIComponent(calId(negocio))}/events/${eventoId}`, {
    method: 'PATCH',
    body: JSON.stringify({ extendedProperties: { private: campos || { recordatorio24Enviado: 'si', recordatorio2Enviado: 'si' } } }),
  });
}
