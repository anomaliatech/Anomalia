...
async function pedir(url, opciones, reintentos = 2) {
  const r = await fetch(url, opciones);
  const texto = await r.text();
  if (!r.ok) {
    // 429 (límite) y 5xx: reintenta esperando lo que el proveedor pida (+margen).
    // Ojo: si se espera MENOS de lo que el proveedor indicó, el reintento vuelve a
    // fallar seguro. Por eso el margen y un tope más alto que la espera típica.
    if ((r.status === 429 || r.status >= 500) && reintentos > 0) {
      const pedida = Number((texto.match(/try again in ([\d.]+)s/i) || [])[1]) * 1000;
      const espera = (pedida || 2500) + 300;
      await new Promise((res) => setTimeout(res, Math.min(espera, 12000)));
...
async function compatibleOpenAI(system, messages, tools) {
  // Cada proveedor usa SU clave. Con "OPENAI_API_KEY || GROQ_API_KEY", si algún día
  // hay OPENAI_API_KEY en el entorno por otra cosa, el chat se la mandaría a Groq
  // (que responde 401) y caería entero sin decir por qué.
  const p = proveedorActual();
...
function respuestaPrueba(messages) {
  ...
  if (ultTool && ultTool.name === 'ver_huecos') {
    ...
    return { text: null, toolCalls: [{ id: 't2', name: 'reservar_cita', args: { slotId: 0, servicio: 'Primera consulta', lead: { nombre: 'Ana Prueba', empresa: 'ACME', email: 'ana@ejemplo.com', telefono: '600111222', servicio: 'Primera consulta' } } }] };
  }
  if (ultTool && ultTool.name === 'reservar_cita') {
    return { text: 'Listo, tu cita queda confirmada. Te llega un email con los detalles.', toolCalls: [] };
  }
  if (/\b(cita|reserv|hueco|agend|disponib)/i.test(ultUser)) {
    return { text: null, toolCalls: [{ id: 't1', name: 'ver_huecos', args: { servicio: 'Primera consulta' } }] };
  }
