...
function respuestaPrueba(messages) {
  ...
  if (ultTool && ultTool.name === 'ver_huecos') {
    ...
    if (!nombre || !email) return { text: 'Tengo huecos. ¿Me dices tu nombre y tu email y cuál te viene mejor?', toolCalls: [] };
    return { text: null, toolCalls: [{ id: 't2', name: 'reservar_cita', args: { slotId: 0, servicio, lead: { nombre, email, servicio } } }] };
  }
  if (ultTool && ultTool.name === 'reservar_cita') {
    let ok = false;
    try { ok = !!JSON.parse(ultTool.content).ok; } catch {}
    return { text: ok ? 'Listo, tu cita queda confirmada. Te llega un email con los detalles.'
