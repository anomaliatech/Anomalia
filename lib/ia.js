...
async function compatibleOpenAI(system, messages, tools) {
  const clave = process.env.OPENAI_API_KEY || process.env.GROQ_API_KEY;
  if (!clave) throw new Error(`Falta la clave del proveedor "${proveedorActual()}" (OPENAI_API_KEY o GROQ_API_KEY).`);
  const base = (process.env.IA_BASE_URL || BASE_COMPATIBLE[proveedorActual()] || BASE_COMPATIBLE.openai).replace(/\/$/, '');
